...
import org.litellmkt.handlers.params.HandlerParamsOllamaOptions
...
import org.litellmkt.handlers.ollama.OllamaEmbeddingsHandler
import org.litellmkt.handlers.ollama.OllamaGenerateHandler
...
fun main() {
    ...
    val ollamaHandler: OllamaGenerateHandler = localKoin.koin.get()
    runBlocking {
        ...
        ollamaHandler.stream(
            HandlerParamsOllama(
                ...
                options = HandlerParamsOllamaOptions(
                    temperature = 0.0
                ),
                format = "json",
                stream = false
    ...
    runBlocking {
        ...
        ollamaChat.chat(
            params = HandlerParamsChatOllama(
                model = "llama3.2",
                messages = listOf(
    ...
    runBlocking {
        val ollamaEmbeddings = localKoin.koin.get<OllamaEmbeddingsHandler>()
        ollamaEmbeddings.embeddings(
            params = HandlerEmbeddingsOllamaParams(
                model = "all-minilm",
                input = listOf("Why is the sky blue?", "Why is the grass green?"),
            )
        ).collectLatest {
            println(it)
        }
